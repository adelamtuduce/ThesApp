...
var createDiplomaScheduler = function(request, students, element_id) {
  scheduler.config.xml_date="%Y-%m-%d %H:%i";
    scheduler.config.first_hour = 8;
    scheduler.config.last_hour = 18;
    scheduler.attachEvent("onBeforeTooltip",function(){
  ...
  setInterval(function(){
    ...
    scheduler.init(element_id);
    scheduler.load("/events?enroll_request_id=" + request, "json");
    $.each(students, function(index, values))
    var dp = new dataProcessor("/events/db_action?enroll_request_id=" + request);
    dp.init(scheduler);
    dp.setTransactionMode("GET", false);
}